...
export default function Sidebar({ open, closeSide }) {
    ...
    const role = (roles) =>
        (auth?.role || []).find((r) => r == roles) ? true : false;
    return (
        <div>
            <div className={`w-[264px] h-full py-5 px-0 fixed overflow-x-hidden overflow-y-scroll myscroll top-0 left-0 z-50 shadow-md transition duration-500 lg:translate-x-0 backdrop-blur bg-white/30  ${open ? 'translate-x-0 ease-in' : '-translate-x-[264px] ease-out'}`}>
                <div className="px-4">
